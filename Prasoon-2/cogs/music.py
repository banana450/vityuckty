...
class Music(commands.Cog):
    ...
    async def play_music(self, ctx):
        if len(self.music_queue) > 0:
            ...
            await ctx.reply(embed=discord.Embed(title = "**Playing**",color=Color.blurple(), description =f"""<:stolen_emoji:952432077679259729> {self.music_queue[0][0]['title']} | {self.music_queue[0][2]}""" ))
    ...
    async def p(self, ctx, *args):
        ...
        if voice_channel is None:
            ...
            if type(song) == type(True):
                ...
                embed = discord.Embed(title = "**Music Queue**",color=Color.blurple(), description =f"""<:stolen_emoji:952432077679259729> {song["title"]} | {ctx.author.mention}""")
